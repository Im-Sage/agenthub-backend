...
def create_mock_task_from_message(db: Session, conversation: Conversation, content: str) -> Task | None:
    ...
    agent = get_or_create_agent(db, "mock")
    task = Task(
...
async def broadcast_agent_message(message: Message) -> None:
    message_event = WebSocketMessageEvent(data=MessageRead.model_validate(message))
    await websocket_manager.broadcast_json(message.conversation_id, jsonable_encoder(message_event))

# 这个函数负责执行单个 Agent 任务，并根据执行结果更新任务状态和结果摘要。
# 如果 create_reply_message 参数为 True，还会在对应的对话中创建一条新的消息，
# 内容为 Agent 的执行结果摘要，并通过 WebSocket 广播给所有连接到该对话的客户端。
...
# 这个函数是一个简单的包装器，用于执行一个模拟 Agent 任务。
# 它调用 run_single_agent_task 函数，并将 create_reply_message 参数设置为 True，
# 以确保在任务执行完成后创建一条新的消息并广播给客户端。这可以用于测试和验证整个任务执行流程，
# 包括数据库更新和 WebSocket 广播。
async def run_mock_agent_task(task_id: int) -> None:
    await run_single_agent_task(task_id, create_reply_message=True)


# 这个函数负责执行一个 Orchestrator 任务。它首先将父任务状态更新为 RUNNING，
# 然后查询所有子任务的 ID，并依次执行每个子任务。
# 在所有子任务执行完成后，它会检查是否有任何子任务执行失败，
# 如果有，则将父任务状态更新为 FAILED，并记录错误信息。
# 如果所有子任务都成功执行，则将父任务状态更新为 SUCCESS，
# 并生成一个结果摘要，最后创建一条新的消息包含结果摘要，
# 并通过 WebSocket 广播给所有连接到该对话的客户端。
